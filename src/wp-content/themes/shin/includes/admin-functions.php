 ...
 * @return void
 */
function shin_fail_load_admin_notice() {
	...
	if ( 'true' === get_user_meta( get_current_user_id(), '_shin_install_notice', true ) ) {
		return;
	}
	...
	<script>
		window.addEventListener( 'load', () => {
			...
			dismissNotice.addEventListener( 'click', async ( event ) => {
				...
				formData.append( 'action', 'shin_set_admin_notice_viewed' );
				formData.append( 'dismiss_nonce', '<?php echo esc_js( wp_create_nonce( 'shin_dismiss_install_notice' ) ); ?>' );
 ...
 * @return void
 */
function ajax_shin_set_admin_notice_viewed() {
	check_ajax_referer( 'shin_dismiss_install_notice', 'dismiss_nonce' );

	update_user_meta( get_current_user_id(), '_shin_install_notice', 'true' );
	die;
}
add_action( 'wp_ajax_shin_set_admin_notice_viewed', 'ajax_shin_set_admin_notice_viewed' );

if ( ! did_action( 'elementor/loaded' ) ) {
	add_action( 'admin_notices', 'shin_fail_load_admin_notice' );
}
