...
if ( defined( 'ABSPATH' ) && ! class_exists( 'Change_WP_Admin_Login' ) ) {
	...
	class Change_WP_Admin_Login {
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'change-wp-admin-login' ), '<strong>' . __( 'Change URL login', 'change-wp-admin-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'Change URL login', 'change-wp-admin-login' ) . '</h3>';
		...
		public function admin_init() {
			...
			add_settings_section(
				...
				__( 'Change URL login', 'change-wp-admin-login' ),
